...
import { fetchAnalysis } from "../redux/actions"
...
export default function Analyses() {
  ...
  const [xScale2, setxScale2] = useState({ key: "hour", value: "Heure" })
  ...
  useEffect(() => {
    dispatch(fetchAnalysis(start_time, end_time))
  }, [start_time, end_time])
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, { total_earning, total_earning_last_contract }) => (
        <div>
          <Text type="16-600">{total_earning.toLocaleString("fr")} €</Text>{" "}
          <br />
          <Text type="16-600" className="text-gray-6f">
            {total_earning_last_contract.toLocaleString("fr")}€ ancien contrat
          </Text>
        </div>
      ),
    },
    {
      ...
      render: (_, { total_earning_delta }) => (
        <DeltaBadge background={false} delta={Math.round(total_earning_delta * 100)} />
      ),
    },
    {
      title: "Portefeuille",
      ...
      render: (_, { portfolio_earning, portfolio_earning_last_contract }) => (
        <div>
          ...
          <br />
          <Text type="16-600" className="text-gray-6f">
            {portfolio_earning_last_contract.toLocaleString("fr")}€ ancien
            contrat
          </Text>
        </div>
      ),
    },
    {
      ...
      render: (_, { portfolio_earning_delta }) => (
        <DeltaBadge background={false} delta={Math.round(portfolio_earning_delta * 100) } />
      ),
    },
    {
      ...
      render: (_, { volumes_traded, volumes_traded_last_year }) => (
        <div>
          <Text type="16-600">{volumes_traded.toLocaleString("fr")} Kwh</Text>{" "}
          <br />
          <Text type="16-600" className="text-gray-6f">
            {volumes_traded_last_year.toLocaleString("fr")}Kwh en{" "}
            {d?.reference_year - 1}
          </Text>
        </div>
      ),
    },
    {
      ...
      render: (_, { volumes_traded_delta }) => (
        <DeltaBadge background={false} delta={Math.round(volumes_traded_delta * 100)} />
      ),
    },
  ]
  return (
      ...
      <Spin spinning={analyse.loading}>
        <div className="flex-wrap" style={{ display: "flex" }}>
          ...
          >
            ...
            <div>
              <Text type="32-600" className="block mb-1.5">
                {d?.current_portfolio.total_portfolio.toLocaleString("fr")} €
              </Text>
              <Text type="16-600" className="text-gray-6f">
                {d?.current_portfolio.last_portfolio.toLocaleString("fr")}€ avec
                l’ancien contrat
              </Text>
            </div>
          </Card>
